...
const withinTimeSlot = (item, index) => {
    ...
    return (
        item.start === timeSlotStart || item.end === timeSlotEnd) ||    // service action starts or ends on the start or end of the time slot
        (item.start < timeSlotStart && item.end > timeSlotEnd);         // service action ongoing and time slot falls within its start and end time 
}

export const generateEngineerItems = (items = []) => {
    const { defaultTimeStart, defaultTimeEnd, interval } = config;

    const timeSlots = (defaultTimeEnd.valueOf() - defaultTimeStart.valueOf()) / interval;
    ...
    return [...Array(timeSlots).fill({}).map((_, index) => {
        ...
        return {
            ...
            group: "0",
            title: engineers,
            start: moment(defaultTimeStart).add(12 * index, 'hours').valueOf(),
            end: moment(defaultTimeStart).add(12 + 12 * index, 'hours').valueOf(),
            className: 'engineer',
            canMove: false
        }
    })];
